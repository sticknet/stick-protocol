...
static int load_session_func(signal_buffer **record, signal_buffer **user_record, const signal_protocol_address *address, void *user_data) {
    id <SessionStore> sessionStore = (__bridge id<SessionStore>)(user_data);
...
static int get_identity_key_pair(signal_buffer **public_data, signal_buffer **private_data, void *user_data) {
    id <IdentityKeyStore> identityKeyStore = (__bridge id<IdentityKeyStore>)(user_data);
    KeyPair *keyPair = [identityKeyStore getIdentityKeyPair];
    if (!keyPair) {
        return -1;
    }
...
static int store_sender_key(const signal_protocol_sender_key_name *sender_key_name, uint8_t *record, size_t record_len, uint8_t *user_record, size_t user_record_len, void *user_data) {
    id <SenderKeyStore> senderKeyStore = (__bridge id<SenderKeyStore>)(user_data);
   
    NSData *key = [NSData dataWithBytes:record length:record_len];
  SenderKeyName *senderKeyName = [[SenderKeyName alloc] initWithSenderKeyName:sender_key_name];
