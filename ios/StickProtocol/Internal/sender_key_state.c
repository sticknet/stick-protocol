...
int sender_key_state_deserialize(sender_key_state **state, const uint8_t *data, size_t len,
                                 signal_context *global_context) {
    int result = 0;
    Textsecure__SenderKeyStateStructure *state_structure = 0;
...
int sender_key_state_serialize_prepare(sender_key_state *state, Textsecure__SenderKeyStateStructure *state_structure) {
    ...
    if (state->message_keys_head) {
        ...
        state_structure->sendermessagekeys = malloc(
                sizeof(Textsecure__SenderKeyStateStructure__SenderMessageKey *) * count);
        ...
        DL_FOREACH(state->message_keys_head, cur_node) {
            ...
            state_structure->sendermessagekeys[i] = malloc(
...
int
sender_key_state_deserialize_protobuf(sender_key_state **state, Textsecure__SenderKeyStateStructure *state_structure,
                                      signal_context *global_context) {
    int result = 0;
    sender_key_state *result_state = 0;
...
sender_message_key *
sender_key_state_remove_sender_message_key(sender_key_state *state, uint32_t iteration, int isSticky) {
