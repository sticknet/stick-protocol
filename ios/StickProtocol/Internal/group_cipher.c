...
static int group_cipher_get_sender_key(group_cipher *cipher, sender_message_key **sender_key, sender_key_state *state,
                                       uint32_t iteration, int isSticky);
...
void group_cipher_set_decryption_callback(group_cipher *cipher,
                                          int (*callback)(group_cipher *cipher, signal_buffer *plaintext,
...
int group_cipher_encrypt(group_cipher *cipher,
    ...
    if (isSticky) {
        ...
        sender_chain_key_create_message_key(sender_key_state_get_chain_key(state), &message_key);
        sender_key_state_add_sender_message_key(state, message_key);
...
void ratchet_chain(group_cipher *cipher, int steps) {
    ...
    for (int i = 0; i < steps; i++) {
        sender_chain_key_create_message_key(sender_key_state_get_chain_key(state), &message_key);
        sender_key_state_add_sender_message_key(state, message_key);
        sender_chain_key_create_next(sender_key_state_get_chain_key(state), &next_chain_key);
        sender_key_state_set_chain_key(state, next_chain_key);
    }

    signal_protocol_sender_key_store_key(cipher->store, cipher->sender_key_id, record);
...
int group_cipher_get_sender_key(group_cipher *cipher, sender_message_key **sender_key, sender_key_state *state,
                                uint32_t iteration, int isSticky) {
    int result = 0;
    ...
    if (sender_chain_key_get_iteration(chain_key) > iteration) {
        if (sender_key_state_has_sender_message_key(state, iteration)) {
            result_key = sender_key_state_remove_sender_message_key(state, iteration, isSticky);
            if (!result_key) {
